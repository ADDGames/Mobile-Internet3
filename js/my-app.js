    ...
    success: function(response) {
      ...
      if (response.status === "fail") {
        ...
        $.each(response.data, function(index) {
          ...
          $('#lijstsessiestudent').append("<li id='" + response.data[index].values.SES_id + "'><a href='STU_Sessie.html' class='item-link item-content'><div class='item-inner'><div class='item-title-row'><div class='item-title'>" + response.data[index].values.SES_naam + "</div><div class='item-after'>" + response.data[index].values.SES_actief + "</div><div><div class='item-text'>Gesloten op: " + response.data[index].values.SES_eindtijd + "</div><div></a></li>");
        });
      }
    }
  });
