    ...
    if ($table === 'vakstudent') {
        if ($function === 'getallforvak') {
          ...
          while ($row = mysqli_fetch_array($result)) {
              array_push($sessies, ["index" => $index, "values" => ["SES_id" => $row['SES_id'],"SES_naam" => $row['SES_naam'], "SES_eindtijd" => $row['SES_eindtijd'], "SES_actief" => $row['SES_actief'], "SES_vak_id" => $row['SES_vak_id'], "VAK_naam" => $row['VAK_naam'], "SST_student_id" => $row['SST_student_id'], "STU_id" => $row['STU_id'], "Stu_naam" => $row['GEB_naam'], "Stu_voornaam" => $row['GEB_voornaam']]]);
              $index++;
          }
